...
drawBoard <- function(board, options, turn, selected){
  gridsize <- 8
  if(options[2])
    gridsize <- 10
  
  if(options[3]>0)
    plot(0, 0, type = "n", xlim = c(0, gridsize+1), ylim = c(0, gridsize+1), col="white", xlab = ifelse(turn==options[4], "Human's turn", "Computer's turn"), ylab = "", axes = FALSE, frame.plot = FALSE)  
  else
    plot(0, 0, type = "n", xlim = c(0, gridsize+1), ylim = c(0, gridsize+1), col="white", xlab = paste("Player ", turn, "'s turn", sep=""), ylab = "", axes = FALSE, frame.plot = FALSE)
  ...
  for(i in 1:gridsize)
    ...
    rect(i, j, i + 1, j+1, col = themeColors[[options[1]+1]][((j%%2 + i%%2) %% 2) + 3], border = "black")
  }
  rect(0, 0, 1, 1, col = "thistle", border = "black")
  if(all(selected > 0))
...
while(!checkwin(board) || floor(click$y) < 2) {
  ...
  if(menu>0 && !checkwin(board)){
    ...
    if(selected[1] == -1)
      ...
      movePiece(board, c(selected, floor(click$x), floor(click$y)), turn)
  ...
  drawBoard(board, options, turn, selected)
